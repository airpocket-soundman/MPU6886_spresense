...
int MPU6886::Init(void){
  ...
  regdata = MPU6886_ACCEL_CONFIG_VALUE;
  ...
  regdata = MPU6886_CONFIG_VALUE;       //default 0x01
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_CONFIG, 1, &regdata);
  delay(1);

  regdata = MPU6886_SMPLRT_DIV_VALUE;   //default 0x05
  ...
  regdata = MPU6886_ACCEL_CONFIG2_VALUE;      //default 0x00
...
void MPU6886::getAccelZAdc(int16_t* az){
   uint8_t buf[6];  
   I2C_Read_NBytes(MPU6886_ADDRESS,MPU6886_ACCEL_ZOUT_H,2,buf);
   
   *az=((int16_t)buf[0]<<8)|buf[1];
}
...
void MPU6886::getAccelZData(float* az){
  int16_t accZ = 0;
  getAccelZAdc(&accZ);
